...
Q_DEMO = 5  # small prime for exact arithmetic
...
def _test_composition():
    p = Q_DEMO
    f = (1, 2, 3, 4)  # det = 4 - 6 = -2 mod 5 = 3
    g = (2, 1, 1, 4)  # det = 8 - 1 = 7 mod 5 = 2
    assert det(*f, p) == 3
    assert det(*g, p) == 2
    a, b, c, d = f
    e, ff, gg, hh = g
...
def _test_infinity_handling():
    p = Q_DEMO
    # f(z) = 1/z: 0 <-> inf
    assert det(0, 1, 1, 0, p) == 4  # -1 mod 5
...
def _test_p1_transitive():
    p = Q_DEMO
    # translations z -> z + b send 0 to b for each b in F_p
    for b in range(p):
