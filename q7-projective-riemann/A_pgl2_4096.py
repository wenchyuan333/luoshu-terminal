...
def pgl2_order(q: int) -> int:
    # PGL = GL / center; |center of GL(2, F_q)| = q - 1
    return sl2_order(q)
...
def _test_orders_f2():
    ...
    assert pgl2_order(q) == 6  # PGL(2, F_2) ≅ S_3
    assert p1_points_count(q) == 3


def _test_sharp_3_transitive_count_f3():
    q = 3
    # sharply 3-transitive: |PGL(2, F_q)| = (q+1) * q * (q-1)
...
def _test_pgl_vs_agl_domain_check():
    # PGL(2, F_q) acts on q + 1 points (P^1)
    # AGL(2, F_q) acts on q^2 points (affine plane)
    q = 3
    assert p1_points_count(q) == 4
    assert q * q == 9  # AGL domain size


def _test_psl_equals_pgl_when_q_even():
    # For F_{2^n}: gcd(2, q-1) = gcd(2, odd) = 1, so PSL = PGL
    for q in (4, 16, 4096):
